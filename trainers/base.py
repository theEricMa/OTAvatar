...
from util.distributed import is_master,get_rank
import threading
...
class BaseTrainer(object):
    ...
    def __init__(self,
        ...
        self.opt = opt
        self.net_Warp = net_Warp
        self.net_G = net_G
        if opt.distributed:
            self.net_Warp_module = self.net_Warp.module
            self.net_G_module = self.net_G.module
        else:
            self.net_Warp_module = self.net_Warp
            self.net_G_module = self.net_G

        self.is_inference = train_data_loader is None
        self.net_Warp_ema = net_Warp_ema
        self.opt_Warp = opt_Warp
        self.sch_Warp = sch_Warp
        self.net_G_ema = net_G_ema
        self.opt_G = opt_G
        self.sch_G = sch_G
        self.train_data_loader = train_data_loader

        self.criteria = dict() #nn.ModuleDict()
        self.weights = dict()
        self.losses = dict(gen_update=dict(), dis_update=dict())
        self.gen_losses_template = {
        }
        self.gen_losses = self.gen_losses_template
        ...
        for loss_name, loss_weight in self.weights.items():
            ...
            if loss_name in self.criteria.keys() and \
                ...
                if isinstance(self.criteria[loss_name], torch.nn.Module):
                    self.criteria[loss_name].to('cuda:{}'.format(get_rank()))
        ...
        self.lpips = LPIPS()
        self.psnr = lambda x, y: 20 * torch.log10(255.0 / torch.sqrt(
            torch.mean((x - y) ** 2)))
    ...
    def _init_tensorboard(self):
        ...
        self.meters = {}
        names = ['optim/warp_lr', 'time/iteration', 'time/epoch', 
                 'metric/best_lpips', 'metric/lpips', 'metric/psnr', 'metric/best_psnr']
        for name in names:
    ...
    def _write_tensorboard(self):
        ...
        self._write_to_meters({'time/iteration': self.time_iteration,
                               ...
                               'optim/warp_lr': self.sch_Warp.get_last_lr()[0]},
                                self.meters)
    ...
    def _write_loss_meters(self):
        ...
        for loss_name, loss in gen_losses.items():
            ...
            if isinstance(loss, list):
                loss = sum(loss) / len(loss) if len(loss) != 0 else torch.tensor(0)
    ...
    def save_checkpoint(self, current_epoch, current_iteration):
        ...
        _save_checkpoint(self.opt,
                         self.net_Warp, self.net_Warp_ema, 
                         self.opt_Warp, self.sch_Warp,
    ...
    def _load_checkpoint(self, checkpoint_path, resume=True):
        ...
        self.net_Warp.load_state_dict(checkpoint['net_Warp'], strict=False)
        self.net_Warp_ema.load_state_dict(checkpoint['net_Warp_ema'], strict=False)
        print('load [net_Warp] and [net_Warp_ema] from {}'.format(checkpoint_path))
        
        if isinstance(self.net_G, torch.nn.Module) and checkpoint['net_G'] is not None:
            self.net_G.load_state_dict(checkpoint['net_G'], strict=False)
        if isinstance(self.net_G_ema, torch.nn.Module) and checkpoint['net_G_ema'] is not None:        
        ...
        if self.opt.phase == 'train' and resume:
            ...
            self.opt_Warp.load_state_dict(checkpoint['opt_Warp'])
            self.sch_Warp.load_state_dict(checkpoint['sch_Warp'])
            if isinstance(self.opt_G, torch.nn.Module) and checkpoint['opt_G'] is not None:
                self.opt_G.load_state_dict(checkpoint['opt_G'])
            if isinstance(self.sch_G, torch.nn.Module) and checkpoint['sch_G'] is not None:
    ...
    def start_of_iteration(self, data, current_iteration):
        ...
        if not self.is_inference:
            self.net_Warp.train()
            if self.opt_G is not None: # net_G will be trained as well
                self.net_G.train()
    ...
    def end_of_iteration(self, data, current_epoch, current_iteration):
        ...
        # iteration mode.
        if self.opt.warp_optimizer.lr_policy.iteration_mode:
            self.sch_Warp.step()
        if hasattr(self.opt, 'gen_optimizer') and self.opt.gen_optimizer.lr_policy.iteration_mode:
    ...
    def _print_current_errors(self):
        ...
        for loss_name, losses in self.gen_losses.items():
            if isinstance(losses, list):
                losses = sum(losses) / len(losses) if len(losses) != 0 else torch.tensor(0)
    ...
    def end_of_epoch(self, data, val_dataset, current_epoch, current_iteration):
        ...
        self.current_iteration = current_iteration
        self.current_epoch = current_epoch
        if not self.opt.warp_optimizer.lr_policy.iteration_mode:
            self.sch_Warp.step()
        if hasattr(self.opt, 'gen_optimizer') and not self.opt.gen_optimizer.lr_policy.iteration_mode:
            if self.sch_G is not None:
                self.sch_G.step()
    ...
    def eval(self, val_dataset):
        ...
        metrics = self.test(val_dataset, output_dir, self.current_iteration)
        self.write_data_tensorboard(
            {'test_lpips': metrics['lpips'], 'test_psnr': metrics['psnr']},
    ...
    def save_image(self, path, data):
        ...
        self.net_Warp.eval()
        self.net_G.eval()
        if hasattr(self, 'net_G_ema') and isinstance(self.net_G_ema, torch.nn.Module):
            self.net_G_ema.eval()
        ...
        # target_image_list, predict_images_list = self._get_visualizations(data)

        # def save_image_thread(target_images,predict_images):
        #     print('start collecting image')
        #     target_vis_images = torch.cat(target_images, dim = 3)
        #     predict_vis_images = torch.cat(predict_images, dim = 3)
        #     vis_images = torch.cat([target_vis_images, predict_vis_images], dim = 2)
        #     vis_images = torch.cat(torch.chunk(vis_images, vis_images.size(0), 0)[:10], 2)
        ...
        # t = threading.Thread(target=save_image_thread, args=(target_image_list, predict_images_list))
        # t.start()
    ...
    def write_metrics(self, data):
        ...
        if cur_metrics is not None:
            ...
            if self.best_psnr is not None:
                self.best_psnr = max(self.best_psnr, cur_metrics['psnr'])  
            else:
                self.best_psnr = cur_metrics['psnr']  
            metric_dict = {
                'metric/lpips': cur_metrics['lpips'], 
                'metric/best_lpips': self.best_lpips,
                'metric/psnr': cur_metrics['psnr'],
                'metric/best_psnr': self.best_psnr,
                }
...
def _save_checkpoint(opt,
                     net_Warp, net_Warp_ema, opt_Warp, sch_Warp,
    ...
    torch.save(
        {
            'net_Warp': net_Warp.state_dict(),
            'net_Warp_ema': net_Warp_ema.state_dict(),
            'opt_Warp': opt_Warp.state_dict(),
            'sch_Warp': sch_Warp.state_dict(),
            'net_G': net_G.state_dict() if isinstance(net_G, torch.nn.Module) else None,
            'net_G_ema': net_G_ema.state_dict() if isinstance(net_G_ema, torch.nn.Module) else None,
            'opt_G': opt_G.state_dict() if isinstance(opt_G, torch.nn.Module) else None,
            'sch_G': sch_G.state_dict() if isinstance(sch_G, torch.nn.Module) else None,
